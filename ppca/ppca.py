...
# terminology from Ilin & Raiko (2010)
# X: c by n, W: d by c, Y: d by n  
# Y = WX - m*1

# feed a numpy matrix into ProbPCA(data=...)
...
class ProbPCA:
    ...
    def fit_transform(self, n_components=5): 
        ...
        if missing_values:
            ...
            print "No missing values, go use a faster vanilla PCA."
    ...
    def _fit_transform_missing(self):
        ...
        self.yhat = self.data.copy()
        self.yhat[np.isnan(self.yhat)] = 0
        ...
        while supnorm > 1e-3:
            ...
            var_y = self._update_vary(W0, cov_x, mean_x, mean_y)
            yhat = np.dot(W0, mean_x.squeeze(axis=1)) + np.tile(mean_y, (1, self.n))
            supnorm = np.max(np.abs(self.yhat-yhat))
            self.components = mean_x.squeeze(axis=1)
            self.yhat = yhat
